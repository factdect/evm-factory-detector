...
export function openDb(file = process.env.DB_PATH || './data/factory.db') {
  ...
  const q = {
    ...
    putToken: db.prepare(`INSERT OR REPLACE INTO tokens (
        ...
        name, symbol, decimals, total_supply, is_lp, created_at)
  ...
  const ATTRIBUTION = ['launchpad_id', 'confidence', 'factory', 'factory_label', 'event_sig', 'event_fields', 'family', 'birth_block', 'birth_tx', 'birth_source'];
